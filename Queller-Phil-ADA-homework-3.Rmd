...
ci <- predict(fit,
  newdata = data.frame(log_Body_mass_female_mean = seq(min(log_df$log_Body_mass_female_mean, na.rm = TRUE), max(log_df$log_Body_mass_female_mean, na.rm = TRUE), by = 1)),
  interval = "confidence", level = 1 - alpha
)
ci <- data.frame(ci)
ci <- cbind(seq(min(log_df$log_Body_mass_female_mean, na.rm = TRUE), max(log_df$log_Body_mass_female_mean, na.rm = TRUE), by = 1), ci)
